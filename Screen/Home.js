 ...
 // Delete API 
  const hadleRemove = async () => {
    try {
      ...
      const response = await axios.delete(`https://localhost:44366/api/DeleteEmployee/{id}`);
 ...
 // Post Api 
  ...
  return (
    <SafeAreaView>
      ...
      <Text style={styles.txtMain} >Stud {data.length}</Text>
       
      <TouchableOpacity onPress={handleCreate} >
        <Text style={styles.txtMain}>New</Text>
      </TouchableOpacity>

       <TouchableOpacity onPress={()=> navigation.navigate('Update')}>
               <Text style={styles.txtMain}>Update</Text>
      </TouchableOpacity>

      {/* <TouchableOpacity onPress={()=> navigation.navigate('search')}>
          <Text style={styles.txtMain}>Search</Text>
      </TouchableOpacity> */}
